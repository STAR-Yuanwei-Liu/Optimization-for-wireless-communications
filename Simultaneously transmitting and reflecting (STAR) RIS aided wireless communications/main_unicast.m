...
for combination=1:1:5% ranging M from 10 to 50
    ...
    for realization=1:1 %channel realizations
        ...
        %%%%%%%%%%%%%%%% Energy splitting
        %Initialized a random passive beamforming and find feasible W
        Q_r_ES0=1/2*Q_r0;
        cvx_begin
        variable W_ES0(N,N,K) Hermitian semidefinite
        expressions p_ES0(K)
        for k=1:K
            p_ES0(k)=real(trace(W_ES0(:,:,k)));
        end
        minimize sum(p_ES0)
        subject to
        for k=1:K
            ...
            QoS(k)*real(trace(Q_r_ES0(:,:,k)*H(:,:,k)*W_ES0(:,:,j)*H(:,:,k)'))+QoS(k)-real(trace(Q_r_ES0(:,:,k)*H(:,:,k)*W_ES0(:,:,k)*H(:,:,k)'))<=0;
        end
        cvx_end
        if(strcmp(cvx_status,'Infeasible')||strcmp(cvx_status,'Failed'))
            consum_power_ES(realization,combination)=100;
            break;
        end
        ...
        for outer=1:maxOut
            ...
            for inter=1:maxIn
                ...
                for k=1:K
                    ...
                    QoS(k)/2*pow_pos(norm(Q_ES(:,:,k)+H(:,:,k)*W_ES(:,:,j)*H(:,:,k)','fro'),2)+1/2*pow_pos(norm(Q_ES(:,:,k)-H(:,:,k)*W_ES(:,:,k)*H(:,:,k)','fro'),2)+(QoS(k)+1)/2*pow_pos(norm(Q_r_ES(:,:,k),'fro'),2)-(QoS(k)+1)*real(trace(Q_r_ES(:,:,k)'*Q_ES(:,:,k)))+QoS(k)/2*pow_pos(norm(H(:,:,k)*W_r_ES(:,:,j)*H(:,:,k)','fro'),2)-QoS(k)*real(trace((H(:,:,k)'*H(:,:,k)*W_r_ES(:,:,j)*H(:,:,k)'*H(:,:,k))'*W_ES(:,:,j)))+1/2*pow_pos(norm(H(:,:,k)*W_r_ES(:,:,k)*H(:,:,k)','fro'),2)-1*real(trace((H(:,:,k)'*H(:,:,k)*W_r_ES(:,:,k)*H(:,:,k)'*H(:,:,k))'*W_ES(:,:,k)))+QoS(k)<=0;
                end
                for k=1:K
                    for m=1:M
        ...
        Q_r_MS0=1/2*Q_r0;
        cvx_begin
        variable W_MS0(N,N,K) Hermitian semidefinite
        expressions p_MS0(K)
        for k=1:K
            p_MS0(k)=real(trace(W_MS0(:,:,k)));
        end
        minimize sum(p_MS0)
        subject to
        for k=1:K
            ...
            QoS(k)*real(trace(Q_r_MS0(:,:,k)*H(:,:,k)*W_MS0(:,:,j)*H(:,:,k)'))+QoS(k)-real(trace(Q_r_MS0(:,:,k)*H(:,:,k)*W_MS0(:,:,k)*H(:,:,k)'))<=0;
        end
        cvx_end
        if(strcmp(cvx_status,'Infeasible')||strcmp(cvx_status,'Failed'))
        ...
        for outer=1:maxOut
            ...
            for inter=1:maxIn
                ...
                variable W_MS(N,N,K) Hermitian semidefinite
                variable Q_MS(M,M,K) Hermitian semidefinite
                expressions p_MS(K) penlty_MS(K) penlty_BMS(M)
                for m=1:M
                ...
                for k=1:K
                    ...
                    QoS(k)/2*pow_pos(norm(Q_MS(:,:,k)+H(:,:,k)*W_MS(:,:,j)*H(:,:,k)','fro'),2)+1/2*pow_pos(norm(Q_MS(:,:,k)-H(:,:,k)*W_MS(:,:,k)*H(:,:,k)','fro'),2)+(QoS(k)+1)/2*pow_pos(norm(Q_r_MS(:,:,k),'fro'),2)-(QoS(k)+1)*real(trace(Q_r_MS(:,:,k)'*Q_MS(:,:,k)))+QoS(k)/2*pow_pos(norm(H(:,:,k)*W_r_MS(:,:,j)*H(:,:,k)','fro'),2)-QoS(k)*real(trace((H(:,:,k)'*H(:,:,k)*W_r_MS(:,:,j)*H(:,:,k)'*H(:,:,k))'*W_MS(:,:,j)))+1/2*pow_pos(norm(H(:,:,k)*W_r_MS(:,:,k)*H(:,:,k)','fro'),2)-1*real(trace((H(:,:,k)'*H(:,:,k)*W_r_MS(:,:,k)*H(:,:,k)'*H(:,:,k))'*W_MS(:,:,k)))+QoS(k)<=0;
                end
                for k=1:K
                    for m=1:M
        ...
        for k=1:K
            ...
            W_TS(:,:,k)=H(:,:,k)'*Q_TS(:,:,k)*H(:,:,k)/(real(trace(Q_TS(:,:,k)*H(:,:,k)*H(:,:,k)')));
        end
