%%proposed ESA
%function[swap_or_not,match_c,p,Hc,alpha,do,weak1,weak2,randomornot]=swap_many_2(user,match_c,p,Hc,do,opt_H,first,second,channel,alpha)
%swap blocking pair 
% clear all;
% clc;
%user=6;
...
weak1=log2(1+p_swap(c2,mod(i2,2)+1)*Hc_swap(c2,mod(i2,2)+1)/(p_swap(c2,i2)*Hc_swap(c2,mod(i2,2)+1)+delta)); %weaker user matched with 1
else
...
weak1=log2(1+p_swap(c2,i2)*Hc_swap(c2,i2)/(p_swap(c2,mod(i2,2)+1)*Hc_swap(c2,i2)+delta));  %first user himself
end
%second user
if  do_sec==2  
...
u_total=u_fir_c+u_sec_c;
u_total_new=u_fir_c2+u_sec_c2;
...
if u_fir_u2>=u_fir_u && u_sec_u2>=u_sec_u  && (u_fir_u2>u_fir_u || u_sec_u2>u_sec_u)  &&(u_total_new>u_total)
swap_or_not=1;
randomornot=0;
% %direct change
% Hc=Hc_swap;
...
do(c1,mod(i1,2)+1)=mod(do_sec,2)+1;
        if do_fir==2
            alpha(c2)=(2^weak1-1)/Hc(c2,mod(i2,2)+1)/1e11;
        else
            alpha(c2)=(2^weak1-1)/Hc(c2,i2)/1e11;
        end

        if do_sec==2
            alpha(c1)=(2^weak2-1)/Hc(c1,mod(i1,2)+1)/1e11;
        else
            alpha(c1)=(2^weak2-1)/Hc(c1,i1)/1e11;
        end
